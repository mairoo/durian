...
public class BannedEmail {
  ...
  @Builder
  private BannedEmail(
      Long id, String email, LocalDateTime created, LocalDateTime modified, boolean isRemoved) {
    this.id = id;
    this.email = email;
  ...
  public static BannedEmail of(String email) {
    return BannedEmail.builder().email(email).build();
  }

  public BannedEmailEntity toEntity() {
    return BannedEmailEntity.builder().id(this.getId()).email(this.getEmail()).build();
  }
