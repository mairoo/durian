...
public class AuthService {
  ...
  public TokenPair login(SignInRequest request, HttpServletRequest servletRequest) {
    ...
    return new TokenPair(
        AccessTokenResponse.of(accessToken, jwtProperties.accessTokenExpiresIn()), refreshToken);
  }
  ...
  public TokenPair refresh(String refreshToken, HttpServletRequest servletRequest) {
    ...
    return new TokenPair(
        AccessTokenResponse.of(newAccessToken, jwtProperties.accessTokenExpiresIn()),
        newRefreshToken);
  }

  /** Redis에 리프레시 토큰 관련 정보 저장 - 리프레시 토큰을 키로 하는 해시: 이메일, IP 주소 저장 - 이메일을 키로 하는 문자열: 리프레시 토큰 저장 */
  private void saveRefreshTokenInfo(String refreshToken, String email, HttpServletRequest request) {
    ...
    redisTemplate
        .opsForValue()
        .set(email, refreshToken, jwtProperties.refreshTokenExpiresIn(), TimeUnit.SECONDS);
  }

  /** 리프레시 토큰 유효성 검증 - UUID 형식 검증 - Redis에 저장된 토큰인지 확인 - 요청 IP와 저장된 IP 일치 여부 확인 */
  private void validateRefreshToken(String refreshToken, HttpServletRequest request) {
    if (refreshToken == null
        || !refreshToken.matches(
