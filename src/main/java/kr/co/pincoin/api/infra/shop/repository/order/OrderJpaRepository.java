...
public interface OrderJpaRepository extends JpaRepository<OrderEntity, Long> {
    ...
    Optional<OrderEntity> findByIdAndUserId(@Param("orderId") Long orderId, @Param("userId") Integer userId);

    @Query("SELECT o FROM OrderEntity o " +
            "LEFT JOIN FETCH o.user u " +
            "LEFT JOIN ProfileEntity p ON p.user = u " +
            "WHERE o.orderNo = :orderNo AND u.id = :userId")
    Optional<OrderEntity> findByOrderNoAndUserId(@Param("orderNo") String orderNo, @Param("userId") Integer userId);
