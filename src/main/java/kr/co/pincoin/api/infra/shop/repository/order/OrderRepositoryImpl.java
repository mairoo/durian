...
public class OrderRepositoryImpl implements OrderRepository {
  ...
  public Optional<Order> findByIdWithUser(Long orderId) {
    return orderJpaRepository.findByIdWithUser(orderId).map(orderMapper::toModel);
  }

  @Override
  public Optional<Order> findByIdAndUserId(Long orderId, Integer userId) {
    return orderJpaRepository.findByIdAndUserId(orderId, userId).map(orderMapper::toModel);
  }

  @Override
  public Optional<Order> findByOrderNoAndUserId(String orderNo, Integer userId) {
    return orderJpaRepository.findByOrderNoAndUserId(orderNo, userId).map(orderMapper::toModel);
  }
  ...
  public Page<Order> searchOrders(OrderSearchCondition condition, Pageable pageable) {
    ...
    List<Order> orders =
        orderEntities.getContent().stream().map(orderMapper::toModel).collect(Collectors.toList());
