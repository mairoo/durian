...
public class GlobalExceptionHandler {
  ...
  // 1. 비즈니스 / 애플리케이션 예외
  /** 비즈니스 로직 예외 처리 애플리케이션에서 정의한 비즈니스 규칙 위반 시 발생하는 예외 처리 예) 잔액 부족, 재고 부족, 주문 취소 불가 등 */
  @ExceptionHandler(BusinessException.class)
  protected ResponseEntity<ErrorResponse> handleBusinessException(
      BusinessException e, HttpServletRequest request) {
    log.error("[Business Exception] {}", e.getMessage());
    return ResponseEntity.status(e.getErrorCode().getStatus())
        .body(ErrorResponse.of(request, e.getErrorCode().getStatus(), e.getMessage()));
  }
  ...
  // 3. 입력값 검증 예외
  /** 입력값 유효성 검사 실패 처리 Bean Validation (@Valid, @Validated) 실패 시 발생하는 예외 처리 */
  ...
  protected ResponseEntity<ErrorResponse> handleHttpMessageNotReadableException(
    ...
    return ResponseEntity.status(ErrorCode.REQUEST_BODY_MISSING.getStatus())
        .body(
            ErrorResponse.of(
                request,
  ...
  protected ResponseEntity<ErrorResponse> handleMissingRequestCookieException(
    ...
    return ResponseEntity.status(ErrorCode.REQUEST_COOKIE_MISSING.getStatus())
        .body(
            ErrorResponse.of(
                request,
  ...
  /** HTTP 메소드 오류 처리 지원하지 않는 HTTP 메소드 호출 시 발생하는 예외 처리 */
  ...
  /** Content-Type 오류 처리 지원하지 않는 Content-Type으로 요청 시 발생하는 예외 처리 */
  ...
  /** 파일 업로드 크기 제한 초과 처리 설정된 최대 파일 크기를 초과하여 업로드 시 발생하는 예외 처리 */
  ...
  // 5. DB / 데이터 관련 예외
  /** 엔티티 조회 실패 처리 JPA에서 엔티티를 찾지 못했을 때 발생하는 예외 처리 */
    ...
    if (e.getCause() instanceof SQLIntegrityConstraintViolationException cause) {
      return switch (cause.getErrorCode()) {
        case 1062 -> // Duplicate entry (UK/PK violation)
            ResponseEntity.status(ErrorCode.DUPLICATE_KEY.getStatus())
                .body(
                    ErrorResponse.of(
                        request,
        ...
        case 1452 -> // FK violation
            ResponseEntity.status(ErrorCode.FOREIGN_KEY_VIOLATION.getStatus())
                .body(
                    ErrorResponse.of(
                        request,
        ...
        default ->
            ResponseEntity.status(ErrorCode.DATA_INTEGRITY_VIOLATION.getStatus())
                .body(
                    ErrorResponse.of(
                        request,
  ...
  /** 기타 모든 예외 처리 위에서 처리되지 않은 모든 예외를 처리하는 마지막 단계 */
  ...
  /** 내부 서버 오류 공통 처리 */
  private ResponseEntity<ErrorResponse> handleInternalServerError(
